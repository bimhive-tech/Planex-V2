...
// The party roster (Settings -> Master Data): every company this company
// works with, in any role. Same shape as SimpleMasterList, plus the
// phone/email that make these worth storing once: picking one in the project
// form fills its contact details in too.
...
export function PartyMasterList({ companyId }: Props) {
  const [modalOpen, setModalOpen] = useState(false);
  const [editing, setEditing] = useState<PartyRow | null>(null);
  const [actionError, setActionError] = useState<string | null>(null);

  const { data, loading, error, reload } = useFetch(
    () => api.get<Paginated<PartyRow>>(`/parties/${companyQuery(companyId, { page_size: "200" })}`),
    [companyId],
  );
  ...
  async function handleDelete(row: PartyRow) {
    ...
    setActionError(null);
    try {
      await api.del(`/parties/${row.id}/${companyQuery(companyId)}`);
      reload();
    } catch (err) {
      ...
      setActionError(err instanceof ApiError ? err.message : "Couldn't delete this party.");
    }
  }
    ...
    <div>
      <div className={styles.toolbar}>
        <span className={styles.muted}>
          {data ? `${data.count} ${data.count === 1 ? "party" : "parties"}` : "parties"}
        </span>
        <Button
          size="sm"
      ...
      <div className={styles.surface} style={COLS}>
        ...
        <StateView
          ...
          emptyText="Add a company so it shows up in every role's dropdown on the project form."
          onRetry={reload}
        >
          {rows.map((r) => (

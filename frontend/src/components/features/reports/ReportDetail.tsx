...
// Report Builder: edit this report's content (date, number, description), manage
// its images (cover / progress / attachments), and see a live PDF preview.
...
type Form = {
  title: string; report_number: string; report_date: string;
  period_start: string; period_finish: string; status: string; description: string;
};
...
export function ReportDetail({ reportId, canManage }: { reportId: string; canManage: boolean }) {
  ...
  const { loading, error, reload } = useFetch(async () => {
    const r = await api.get<ReportRow>(`/reports/${reportId}/`);
    setProjectId(r.project);
    setForm({
